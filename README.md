...
The rest of this readme is unchanged from upstream, change "mjenz/rpi-openvpn" to "mjenz/rpi-openvpn" to use the commands.
...
* Initialize the `$OVPN_DATA` container that will hold the configuration files
  ...
  private key used by the newly generated certificate authority.
        ...
        docker run -v $OVPN_DATA:/etc/openvpn --rm mjenz/rpi-openvpn ovpn_genconfig -u udp://VPN.SERVERNAME.COM
        docker run -v $OVPN_DATA:/etc/openvpn --rm -it mjenz/rpi-openvpn ovpn_initpki

* Start OpenVPN server process

        docker run -v $OVPN_DATA:/etc/openvpn -d -p 1194:1194/udp --cap-add=NET_ADMIN mjenz/rpi-openvpn

* Generate a client certificate without a passphrase

        docker run -v $OVPN_DATA:/etc/openvpn --rm -it mjenz/rpi-openvpn easyrsa build-client-full CLIENTNAME nopass

* Retrieve the client configuration with embedded certificates

        docker run -v $OVPN_DATA:/etc/openvpn --rm mjenz/rpi-openvpn ovpn_getclient CLIENTNAME > CLIENTNAME.ovpn
...
Initialize the volume container using the `mjenz/rpi-openvpn` image with the
...
To generate a client certificate, `mjenz/rpi-openvpn` uses EasyRSA via the
`easyrsa` command in the container's path.  The `EASYRSA_*` environmental
variables place the PKI CA under `/etc/openvpn/pki`.

Conveniently, `mjenz/rpi-openvpn` comes with a script called `ovpn_getclient`,
...
run time image (mjenz/rpi-openvpn) and the `$OVPN_DATA` data volume. To remove
